...
void* readJoystickRoutine(void* _fd){
    ...
    while(1){
        ...
        /*if(left != -1){
            ...
            packet.translational_velocity=v_x_norm; 
        }
        if(right != -1){
            v_y_norm = ((float)v_y) / 32767;
            packet.rotational_velocity=v_y_norm; 
        }*/
        if(left != -1){
            packet.translational_velocity = abs((v_x%255));
        }
        if(right != -1){
            packet.rotational_velocity = abs((v_y % 255));
        }
        flag_comm = 1;
